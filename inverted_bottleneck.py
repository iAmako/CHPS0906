...
class my_conv(nn.Module):
    ...
    """
    def __init__(self, in_size, out_size, kernel_size=3, stride=1, padding=1, group = 1):
        super().__init__()
        self.conv = nn.Conv2d(in_size, out_size, kernel_size=kernel_size, stride=stride, padding=padding, groups=group)
...
class inverted_bottleneck(nn.Module):
    """Structure en inverted bottleneck à utiliser dans un réseau 
    ...
    def __init__(self, in_size=3, hidden_dim=32, out_size=10, kernel_size=3):
        super().__init__()
        #Réduction
        self.conv_a = my_conv(in_size, hidden_dim, 1)
        #Maintient
        self.conv_b = my_conv(hidden_dim, hidden_dim, kernel_size, group=hidden_dim)
        #Augmentation
        self.conv_c = my_conv(hidden_dim, out_size, 1)
...
class bottleneck_stack(nn.Module):
    def __init__(self, in_size=3, hidden_dim=32, out=10, kernel_size=3):
        super().__init__()
        self.bottleneck_a = inverted_bottleneck(in_size, hidden_dim, hidden_dim, kernel_size=1)
        self.bottleneck_b = inverted_bottleneck(hidden_dim, hidden_dim, hidden_dim, kernel_size)
        self.bottleneck_c = inverted_bottleneck(hidden_dim, hidden_dim, out, kernel_size)
